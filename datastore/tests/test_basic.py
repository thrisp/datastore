...
class TestDatastoreCollection(TestDatastore):
    ...
    def test_sharded(self, numelems=1000):
        ...
        hashfn = lambda key: ((int(key.name) * len(stores)) / numelems)
        sharded = ShardedDatastore(stores, shardingfn=hashfn)
        sumlens = lambda stores: sum(map(lambda s: len(s), stores))

        def checkFor(key, value, sharded, shard=None):
            correct_shard = sharded._stores[int(hashfn(key)) % len(sharded._stores)]
        ...
        for value in range(0, numelems):
            key = Key('/fdasfdfdsafdsafdsa/{}'.format(value))
            shard = stores[int(hashfn(key)) % len(stores)]
        ...
        for value in range(0, numelems):
            key = Key('/fdasfdfdsafdsafdsa/%d' % value)
            incorrect_shard = stores[int(hashfn(key) + 1) % len(stores)]
            checkFor(key, value, sharded)
        ...
        for value in range(0, numelems):
            key = Key('/fdasfdfdsafdsafdsa/%d' % value)
            incorrect_shard = stores[int(hashfn(key) + 1) % len(stores)]
            correct_shard = stores[int(hashfn(key)) % len(stores)]
            checkFor(key, value, sharded, incorrect_shard)
            sharded.put(key, value)
